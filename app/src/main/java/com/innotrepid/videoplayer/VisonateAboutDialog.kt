    ...
    AlertDialog(
        ...
        text = {
            LazyColumn(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                item { Text("Momentum is the intelligence layer behind Visonate. It observes viewing behavior, interprets signals, predicts likely next actions, presents those predictions and learns from what you accept or ignore. The goal is anticipation without taking control away from you.", color = MaterialTheme.colorScheme.onSurfaceVariant) }
                item { AboutHeading("Privacy & data") }
                item { Text("Your video files are not copied into the app's private library. Library metadata, playback history, Momentum events and learned prediction preferences are stored locally. Diagnostics leave the device only when you explicitly export them.", color = MaterialTheme.colorScheme.onSurfaceVariant) }
                item { AboutHeading("License") }
                item { Text("Visonate is proprietary software. © 2026 Innotrepid. All rights reserved. See the repository LICENSE file for the full terms.", color = MaterialTheme.colorScheme.onSurfaceVariant) }
                item { Spacer(Modifier.height(2.dp)); Text("Developed by Innotrepid", style = MaterialTheme.typography.labelLarge) }
            }
        }
    )
}

@Composable private fun AboutHeading(text: String) { Text(text, style = MaterialTheme.typography.titleMedium) }
@Composable private fun AboutBullet(text: String) { Text("• $text", color = MaterialTheme.colorScheme.onSurfaceVariant) }
